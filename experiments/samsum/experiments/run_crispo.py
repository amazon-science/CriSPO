...
# Trainer
save_dir = "data/experiments/samsum/samsum_quickshuffled_critique"
...
overall_score_primary, overall_score, scores, predictions = trainer.evaluate(
    best_prompt_dev,
    dataset=test,
    task_llm=task_llm,
...
trainer.logger.info(
    f"`{best_prompt_dev}` score on test set ({len(test)} articles): `[red]{overall_score_primary:.4f}[/red]`"
)
print(overall_score)

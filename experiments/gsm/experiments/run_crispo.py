...
from experiments.gsm.critique_prompt import GsmCritiquePrompt
...
scores = []
for run in range(3):
    save_dir = f"data/experiments/gsm/claude_instant/critique-train=200-{run}"
    ...
    prompt_score_pairs, best_prompt_dev = trainer.fit(
        ...
        meta_prompt=GsmCritiqueMetaPrompt(),
        meta_llm=ClaudeInstant(temperature=1.0),
        critique_prompt=GsmCritiquePrompt(),
        metric=metric,
